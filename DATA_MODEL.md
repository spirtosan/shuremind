...
quiet_hours (start/end, default 22:00–08:00), default_all_day_time (09:00), default currency (EUR), default reminder offsets per type (D-43: EVENT = [P1D, PT2H] — was empty; ANNIVERSARY = [P14D, P1D]; DEADLINE = [P14D, P7D, P1D]), snooze presets (1h/4h/1d), auto_backup (on, folder URI, keep 7), exact_alarms_opt_in. UI language is not a DataStore key (D-21): LanguageRepository wraps AppCompatDelegate.get/setApplicationLocales() directly, persisted by AppCompat's own autoStoreLocales.

D-43: ReminderRuleRepository writes (`setForTask`) now trigger `RecomputeAndRearm` via `ScheduleChangeNotifier`, in parity with every `TaskRepository` write — previously they didn't, so adding/editing a lead-time reminder only re-armed the alarm on the next unrelated trigger (app reopen/boot/housekeeping), not immediately. D-33-style caveat: an install that had already persisted EVENT's pre-D-43 empty default to DataStore (e.g. via a settings import/restore, which writes every `default_reminder_offsets` key unconditionally) won't pick up the new code default on its own, since a present-but-empty DataStore key wins over the code fallback. `DataStoreSettingsRepository.migrateEventDefaultOffsetsIfNeeded()`, run once at app startup and guarded by a DataStore marker, corrects this: it upgrades EVENT from empty to [P1D, PT2H] only if the stored value is still exactly empty, and never re-runs, so a user's later explicit choice (including choosing empty again) is never overwritten.
